                ...
                <div class="card">
                    ...
                    <div class="card-content">
                        <div class="card-body">
                            ...
                            @foreach(explode("\r\n",$init_seo_key->keyword_site) as $related_val)
                            <div class="alert alert-light" role="alert">
                                <h4 class="alert-heading"><span class="float-right">{{$in->created_at->diffForHumans()}}</span>فعالیت {{$loop->index+1}}</h4>

                                <p class="mb-0">
                                    یه محتوا با کلمه کلیدی <span style="color: red">{{$related_val/*[$loop->index]*/}}</span> بنویس و داخل سایتت قرار بده بعدش یه لینک با کلمه <span style="color: green">{{$val}}</span> بده به صفحه
                                    {{$val}}که قبلا ساختی. و همچنین یه لینک دیگه با عنوان {{$val.'در  '.$local_val}}بده به صفحه {{$val.'در  '.$local_val}}که ساخته بودی. :)
                                </p>
                                <div class="row">
                                    <div class="col-lg-1 col-md-12">
                                        <button type="button"
